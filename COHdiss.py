...
def main():
    ...
    eact_coh_form_f_list = [166, 115, 218, 193, 203, 232, 228, 194, 207]
    eact_coh_form_b_list = [52, 23, 110, 36, 125, 6, 134, 103, 78]
    eact_coh_diss_f_list = [77, 57, 120, 123, 100, 88, 106, 26, 52]
    ...
    rates = []
    for eads_co, eads_h2, eact_coh_form_f, eact_coh_form_b, eact_coh_diss in \
        zip(eads_co_list, eads_h2_list, eact_coh_form_f_list, 
            eact_coh_form_b_list, eact_coh_diss_f_list):
        # perform time integration
        t,y = solve_odes(T, eads_co*1e3, eads_h2*1e3, eact_coh_form_f*1e3, 
                         eact_coh_form_b*1e3, eact_coh_diss*1e3, pt)
        ...
        rate = calc_k_arr(T, 1e13,  eact_coh_diss*1e3) * y[-1,2]
    ...
    plt.xlabel('Surface')
    plt.ylabel('log(r)')
    plt.title('COH mechanism')
    plt.show()
    
    for label,r in zip(surfaces, rates):
        print("%s: %12.6e" % (label,r))

def solve_odes(T, eads_co, eads_h2, eact_coh_form_f, 
               eact_coh_form_b, eact_coh_diss, pt):
    """
    ...
    eact_coh_form_f     - Forward barrier for COH formation
    eact_coh_form_b     - Reverse barrier for COH formation
    eact_coh_diss       - Forward barrier for COH dissociation
    ...
    r.set_initial_value(y0, t0).set_f_params([T, pa, pb, eads_co, eads_h2, 
                       eact_coh_form_f, eact_coh_form_b, eact_coh_diss])
...
def dydt(t, y, params):
    ...
    eads_co = params[3]
    eads_h2 = params[4]
    eact_coh_form_f = params[5]
    eact_coh_form_b = params[6]
    eact_coh_diss = params[7]
    ...
    kf_3 = calc_k_arr(T, 1e13, eact_coh_form_f)
    kb_3 = calc_k_arr(T, 1e13, eact_coh_form_b)
    kf_4 = calc_k_arr(T, 1e13, eact_coh_diss)
